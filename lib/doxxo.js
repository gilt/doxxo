...
module.exports = function doxxo (options) {
  ...
  function hilite (what) {
    ...
    try {
      if (options.in) {
        extension = path.extname(options.in).substring(1);
        result = highlight.highlight(extension, what, true)
      }
      else {
      ...
      console.log('  Couldn\'t highlight' + ('(' + extension + ')' || '') + ': ' + options.in || '<raw text>');
    }
  ...
  _.each(options.json, function (block) {
    ...
    if (block.code) {
      ...
      if (options.in) {
        block.language = path.extname(options.in).substring(1);
      }
    }
    ...
    if (block.tags) {
      ...
      block.docSection = !!block.notes || !!block.params || !!block.howto || !!block.examples || !!block.ctx;
